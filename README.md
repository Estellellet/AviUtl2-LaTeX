...
`document`テンプレートで「日本語対応」を有効にすると、「一覧から選択」または「ファイルから選択」で日本語フォントを指定できます。この設定は漢字・仮名・日本語の句読点、および数式中の`\text{日本語}`へだけ適用されます。通常の欧文本文と数式フォントはLaTeXの既定値から変更しません。

ディスプレイ数式には`\[ ... \]`を推奨します。既存ソースとの互換性のため`$$ ... $$`もそのまま利用できますが、プラグインによる自動変換は行いません。
